...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>OCCFUG - CFSpreadsheet Magic!</title>
	...
	<cfif isdefined('FORM.submit')>
        ...
        <cfset ConditionalFormattingRule = objCF.createConditionalFormattingRule(1,"3000","4000") />
		...
		<!--- Create a new sheet --->
		<cfset SpreadSheetCreateSheet(sObj, "Coco's Pic")> 
		<!--- Set the sheet as active --->
		<cfset SpreadSheetSetActiveSheet (sObj, "Coco's Pic")>  
		<cfset imgPath = "#ExpandPath( 'files\' )#coolio.jpg"/>
		<cfset imgBytes = FileReadBinary(imgPath) />
		<cfset spreadsheetAddImage(sObj,imgBytes,"jpg","2,2,27,10")>
		
			
		<cfset comment=StructNew()>

		<cfset comment.bold="true">
		<cfset comment.color="blue">
		<cfset comment.comment="My dog think's she's so cool !">
		<cfset comment.linestyle="dotsys">
		<cfset comment.linestylecolor="black">
		<cfset comment.font="Adobe Clean">
		
			
		<cfset spreadsheetSetCellComment(sObj,comment,5,10)>	
	...
	<a href="11_chart_dynamic.cfm">Next</a>	
